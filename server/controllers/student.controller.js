...
const getStudents = async (req, res, next) => {
  try {
    ...
    const [students, total] = await Promise.all([
      Student.find(filter).populate("user", "enrollNumber fullName email").skip(skip).limit(limit),
...
const uploadStudentFiles = async (req, res, next) => {
  ...
  try {
    ...
    if (req.files.resume?.[0]) {
      ...
      if (resume.mimetype !== "application/pdf") throw new AppError("Resume must be a PDF", 400);
      if (resume.size > 2 * 1024 * 1024) throw new AppError("Resume size must be less than 2MB", 400);

      if (student.resume?.public_id) await deleteFromCloudinary(student.resume.public_id, "raw");
      student.resume = await uploadToCloudinary(resume.path, "students/resumes", `${student.id}-resume`, "raw");
    }

    if (req.files.profilePath?.[0]) {
      const profile = req.files.profilePath[0];
      const validTypes = ["image/jpeg", "image/png", "image/jpg"];
      if (!validTypes.includes(profile.mimetype)) throw new AppError("Profile image must be JPG, PNG, or JPEG", 400);
      if (profile.size > 1 * 1024 * 1024) throw new AppError("Profile image must be under 1MB", 400);

      if (student.profilePath?.public_id) await deleteFromCloudinary(student.profilePath.public_id, "image");
      student.profilePath = await uploadToCloudinary(profile.path, "students/profilePics", `${student.id}-profile`, "image");
    }
...
const getStudentFiles = async (req, res, next) => {
  const { id } = req.user;
  try {
    const student = await Student.findOne({ user: id }).select("resume profilePath");
    if (!student) throw new AppError("Student not found", 404);
    res.status(200).json({
      success: true,
      files: { resume: student.resume?.url, profilePath: student.profilePath?.url },
    });
  } catch (error) {
    next(error);
  }
...
const fetchJobById = async (req, res, next) => {
  ...
  try {
    const job = await Job.findById(jobId).populate({ path: "roles.id", select: "roleName" }).select("-recruiter");
...
const applyToJob = async (req, res, next) => {
  ...
  try {
    const student = await Student.findOne({ user: userId }).populate({ path: "user", select: "email name" });
    if (!student) throw new AppError("Create student profile first", 400);
    ...
    if (applications.length === 0) throw new AppError("Already applied for all selected roles", 409);
    ...
    res.status(201).json({ success: true, message: "Successfully applied for roles" });
  } catch (error) {
    next(error);
  }
};
...
const fetchAllAppliedJobs = async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).json({ success: true, message: "Applied jobs fetched successfully", appliedJobs });
  } catch (error) {
    next(error);
  }
};
...
export {
  createStudent, getStudents, getStudent, uploadStudentFiles,
  getStudentVefrification, studentExist, getStudentFiles,
  fetchAllJobs, fetchJobById, applyToJob, fetchAllAppliedJobs
};