...
export const exportRoleApplicantsToCSVService = async (roleId, status) => {
  ...
  if (!applications.length) {
    throw new AppError(
      status === "all"
        ? "No applicants available to export."
        : `No ${status.toLowerCase()} applicants available to export.`,
      400,
    );
  }
